...
A dense configuration in behavior cloning is utilized, which uses a suitable number of actors to prevent typical jam with a large number of actors. 
...
Similar to ResNet-50, each convolution (except the first $ \times $ convolution) is followed by batch normalization and a ReLU activation.
...
\begin{table*}
	\caption{Town02 nocrash results.
		RL-Coach refers to Roach IL and the Expert refers to Roach RL~\cite{Zhang:2021}. 
...
In the empty case, BID significantly outperforms RL-Coach in avoiding traffic light infractions, contributing to a higher S.SR.
This conclusion holds true in the regular case as well. 
In the busy case, BID comes close to matching the expert's performance, again outperforming RL-Coach in terms of S.SR and resulting in fewer collisions with vehicles. 
