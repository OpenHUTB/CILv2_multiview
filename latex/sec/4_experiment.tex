...
\begin{figure}[t]
	...
	\caption{Top: When the ego-vehicle enters a new road segment from an intersection, the ``go-straight" navigation command becomes ambiguous, as the ego-vehicle is legally allowed to move into any of the highlighted lanes.
	Bottom: Four aerial views taken at different times, with the pre-planned global trajectory shown in \textbf{\textcolor{red}{red}}, illustrate how the high-level navigation command shifts to ``move-to-right-lane," guiding the vehicle back to the desired trajectory.}
...
\hspace{1pc}The performance of an imitation learning (IL) agent is inherently limited by the quality of the expert it imitates. 
...
% 一步步添加后的效果
By incorporating the dorsal stream and speed embedding into the baseline, $\mathcal{L}_\text{D}$ outperforms $\mathcal{L}$ overall.
