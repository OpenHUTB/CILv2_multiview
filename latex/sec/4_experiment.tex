...
Specifically, BID chooses to speed out because of the crossing human, despite the green traffic sign and the goal instruction is to turn left. 
...
Given the improvements in our best-performing BID, it is expected that $\mathcal{L}_\text{D}$ and $\mathcal{L}_\text{D} + \mathcal{L}_\text{N}$ gain better SR than results described in Table~\ref{table:sucess_rate_nc_dense}.
...
When $E_\text{s}$ is given merely, the feature required to generate accurate driving action does representation mapping enhance performance.
Since the expected path is given throught nagivation command in this instance, $E_\text{N}$ includes goal instructions.
The use of $E_\text{N}$ improves accuray for the benchmark because it encodes gal instruction, including the feature pointing to the following expected path.
...
Predicting the value should be made easier by imitating the representation of RL-roach, which intuitively contains the feature needed for value prediction. 
...
The issue is significantly reduced by imitating expert leading to a 23\% absolute improvement in the driving score for $\mathcal{L}_\text{D}$.
