...
\begin{table*}
	...
	\resizebox{0.95\linewidth}{!}{
		\begin{tabular}{@{}lccc|ccc|ccc@{}}
			...
			& \textbf{SR(\%,$\uparrow$)} & \textbf{SSR(\%,$\uparrow$)} &  TL($\downarrow$)  & \textbf{SR(\%,$\uparrow$)} & \textbf{SSR(\%,$\uparrow$)} &  TL($\downarrow$) & \textbf{SR(\%,$\uparrow$)} & \textbf{SSR(\%,$\uparrow$)} & CV($\downarrow$) \\
			\hline
...
BID comes close to matching the expert's performance in the crowded scenario, again outperforming RL-Coach in terms of \emph{SSR} and resulting in fewer crashes with cars. 
...
Out of the 3 methods, RL-Coach performs the least, incurring more violations and resulting in a noticeably smaller ADG. 
APA of 98\% attained by BID is comparable to MBI. 
However, MBI has the highest ADG score (73\%), while BID achieves 68\%. 
...
We optimize the BID (Fig.~\ref{fig:score_eu_lb_tt_tn}) and conduct ablation studies with the crowded actors configuration to establish a high precision effect and ensure a effective evaluation (Table~\ref{table:infraction}), allowing the proposed agent to attain a driving grade of 78\% and a SS of 91\%. 
...
% L_N 导航信号
% L_D 背侧流信号
Fig.~\ref{fig:score_eu_lb_tt_tn} shows the behavior rates of experienced and imitation learning drivers for an epoch on offical benchmark with crowded actors.
...
Given the improvements in our best-performing BID, it is expected that $\mathcal{L}_\text{D}$ and $\mathcal{L}_\text{D} + \mathcal{L}_\text{N}$ gain better \emph{SR} than results described in Table~\ref{table:sucess_rate_nc_dense}.
The significant performance gap between the baseline and $\mathcal{L}_\text{D} + \mathcal{L}_\text{N}$, especially when transferring to the scene never seen it before, highlights the basic approach limitations.


% 一步步添加后的效果
$\mathcal{L}_\text{D}$ outperforms $\mathcal{L}_\text{b}$ overall by incorporating the dorsal stream and speed embedding into the baseline $\mathcal{L}_\text{b}$.
Additionally, learning from the action distribution allows $\mathcal{L}_\text{D}$ to generalize superior than $\mathcal{L}_\text{b}$ on the benchmark dataset, but not on the offical leaderboard.
When $E_\text{s}$ is given merely, the feature required to generate accurate driving action does representation mapping to enhance performance.
Since the expected path is given throught nagivation command in this instance, $E_\text{N}$ includes goal instructions.
The use of $E_\text{N}$ improves accuray for the benchmark because it encodes goal instruction, including the feature pointing to the following expected path.
To evaluate this idea, we utilize a integrated model structure in which the goal instruction described as one hot encoding is added to the evaluation feature $E_\text{N}$.
...
%
Adding supervised representation alongside feature matching accelerates the convergence of the process, as demonstrated by $\mathcal{L}_\text{D} + \mathcal{L}_\text{N}$.
However, when feature matching is omitted, supervised representation doesn't lead to better result with $\mathcal{L}_\text{D}$.
...
Interestingly, the remarkably better ``Agent blocked" degree in the basic $\mathcal{L}_\text{A}(\text{a})$ is primarily caused by results from heavy rains. 
The issue is significantly reduced by imitating expert leading to a 23\% absolute improvement in the driving score (DS) for $\mathcal{L}_\text{D}$.
While maintaining the same IL strategy, this gain illustrates the advantages of using a more knowledgeable driver. 
Better improvements are achieved with the addition of soft targets and latent feature supervision in $\mathcal{L}_\text{D}+\mathcal{L}_\text{N}$, resulting in another 30\% absolute increase in performance. 
